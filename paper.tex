...
\title{FedRL-IDS: Federated Reinforcement Learning for Intrusion Detection with Tri-Technique Aggregation and Temporal Reputation-Enhanced Trust}
...
\begin{abstract}
Network Intrusion Detection Systems (NIDS) face critical challenges in distributed IoT/IIoT environments: data heterogeneity (non-IID), privacy constraints, vulnerability to Byzantine adversarial participants, and the need for adaptive detection against evolving attacks. We propose \textbf{FedRL-IDS}, a novel three-tier federated reinforcement learning framework that integrates Proximal Policy Optimization (PPO) with a tri-technique aggregation pipeline combining FLTrust, Fed+, and Dynamic Attention. Our contributions include: (1) a Tier-3 Bernoulli PPO client selector with 8-feature state representation (including gradient alignment and minority class fraction) using hybrid scoring $\mathrm{Score}_k = \pi_{\mathrm{sel}}(k|s) \times \mathrm{Trust}_k \times \mathrm{Attention}_k$; (2) temporal reputation-enhanced trust scoring with growth ($\gamma_r=0.1$) exceeding decay ($\delta_r=0.05$) for Byzantine-robust aggregation; (3) an adaptive entropy coefficient scaled by the number of classes to prevent action collapse in multi-class settings; (4) an MCC-based composite reward function that handles class imbalance without multi-objective reward conflicts; (5) an autoencoder-based novelty detector with periodic retraining for zero-day attack detection; and (6) a CNN-GRU-CBAM backbone architecture for temporal pattern recognition in network flows. We evaluate FedRL-IDS on four benchmark datasets---NSL-KDD, UNSW-NB15, CIC Edge-IIoT 2022, and CIC IoMT 2024---under non-IID federated settings. Results demonstrate that FedRL-IDS achieves 88.90\% accuracy on NSL-KDD with 6.40\% FPR, 99.98\% F1-score on Edge-IIoT, and 96.83\% F1-score on IoMT within 10 communication rounds.
\end{abstract}

\begin{IEEEkeywords}
Federated Learning, Reinforcement Learning, Intrusion Detection System, PPO, CNN-GRU, CBAM, Byzantine Robustness, IoT Security, Dynamic Attention, Client Selection, Meta-Agent, Novelty Detection, Autoencoder, Non-IID
\end{IEEEkeywords}
...
Reinforcement learning, particularly Proximal Policy Optimization (PPO)~\cite{rlintro}, is well-suited for IDS due to its ability to formulate classification as a sequential decision-making problem, directly optimize task-relevant objectives, and adapt to evolving environments. Recent works~\cite{sfedrl, fedrl_attention} have demonstrated competitive intrusion detection using federated RL while preserving data privacy. However, these approaches lack simultaneous mechanisms for Byzantine robustness, personalized aggregation under non-IID data, zero-day attack detection, and intelligent client selection.

\textbf{Limitations of Prior Work.}
Existing federated IDS methods each address only a subset of these challenges. SFedRL-IDS~\cite{sfedrl} introduced hierarchical federated RL but relies on standard FedAvg aggregation (Byzantine-vulnerable and non-IID-sensitive). PriFED-IDS~\cite{prifeds} addresses privacy via differential privacy but provides no trust management. FLTrust~\cite{fltrust} achieves Byzantine robustness using a server-side root dataset but treats each round independently without temporal memory. Fed+~\cite{fedplus} excels at personalization under non-IID settings but operates on supervised learning models. The federated RL framework with dynamic attention~\cite{fedrl_attention} introduces performance-aware weighting but omits both Byzantine robustness and personalization. RL-UDHFL~\cite{rludhfl} incorporated temporal reputation tracking but applied a simplified trust model with decay exceeding growth ($\delta_r > \gamma_r$), structurally biasing reputation toward zero. No existing work simultaneously combines Byzantine robustness, non-IID personalization, fairness-aware attention, and RL-based adaptive detection within a unified federated framework.

\textbf{Our Approach.}
In this paper, we propose \textbf{FedRL-IDS}, a comprehensive three-tier federated reinforcement learning framework for intrusion detection that addresses all three challenges through a unified \emph{tri-technique aggregation pipeline} combining FLTrust, Fed+, and Dynamic Attention. At the core, $N$ distributed PPO agents---each safeguarding a local network segment---are coordinated by a central server that aggregates local model updates through a sequential pipeline. Key innovations include: (1) FLTrust with RL-UDHFL-inspired temporal reputation where growth ($\gamma_r=0.1$) exceeds decay ($\delta_r=0.05$); (2) Fed+ personalization preserving per-client model components under non-IID distributions; (3) Dynamic Attention based on training loss (not accuracy) for fairness; (4) a Tier-3 Bernoulli PPO client selector with 8-feature state and hybrid scoring gated by trust and attention signals; (5) an autoencoder-based novelty detector with periodic retraining for zero-day attack detection; and (6) a CNN-GRU-CBAM backbone architecture for temporal network flow patterns.

Our key contributions are:

\begin{enumerate}
    \item \textbf{Tri-Technique Aggregation}: A novel combination of FLTrust (Byzantine robustness), Fed+ (personalization for non-IID data), and Dynamic Attention (fairness-aware weighting) in a unified pipeline.

    \item \textbf{Temporal Reputation-Enhanced Trust}: FLTrust extended with RL-UDHFL-inspired temporal reputation where growth ($\gamma_r=0.1$) \emph{exceeds} decay ($\delta_r=0.05$), preventing the trust collapse observed when decay dominates.

    \item \textbf{Three-Tier Multi-Agent Architecture}: A hierarchical design with Tier-1 local PPO agents (CNNGRUActor backbone), a Tier-2 meta-agent (CNNGRUActor over one-hot agent actions) for coordinated decision-making, and a Tier-3 Bernoulli PPO client selector for intelligent client selection.

    \item \textbf{RL-Based Client Selection with Hybrid Scoring}: A novel Bernoulli PPO selector with 8-feature state ($R_k, a_k, l_k, \Delta_k, g_k, \mathrm{f1}_k, s_k, m_k$) including gradient alignment $g_k = \cos(\Delta_k, \Delta_{\mathrm{glob}})$ and minority class fraction $m_k$, with hybrid scoring $\mathrm{Score}_k = \pi_{\mathrm{sel}}(k|s) \times \mathrm{Trust}_k \times \mathrm{Attention}_k$.

    \item \textbf{MCC-Based Composite Reward}: A reward function using Matthews Correlation Coefficient (MCC) as the primary signal, supplemented by cost-sensitive TP/TN/FP/FN penalties, novelty bonus, and collapse penalty, eliminating the multi-objective reward conflicts observed with 12+ component rewards.

    \item \textbf{CNN-GRU-CBAM Backbone}: A novel backbone architecture combining Conv1D (multi-scale temporal pattern extraction), CBAM attention (channel and spatial attention), and bidirectional GRU (temporal learning), adapted for 1D network flow sequences.
...
FedRL-IDS implements a \textbf{three-tier architecture}:
...
Tier-2 & Meta-Agent (Coordinator) & CNNGRUActor Categorical \\
...
% Agg internal flow
\draw[arrowstyle=attnorange!80, ->] (fltrust.east) -- node[above, font=\scriptsize]{Trust} (fedplus.west);
\draw[arrowstyle=attnorange!80, ->] (dynattn.west) -- node[above, font=\scriptsize]{Attention} (fedplus.east);

% === Local Agents (bottom) ===
\node[agentbox, below=3.5cm of fltrust, xshift=-1.5cm] (agent1) {Agent 1\\CNNGRU};
\node[agentbox, below=3.5cm of fltrust, xshift=2.5cm] (agent2) {Agent 2\\CNNGRU};
\node[agentbox, below=3.5cm of dynattn, xshift=-2.5cm] (agent3) {Agent 3\\CNNGRU};
\node[agentbox, below=3.5cm of dynattn, xshift=1.5cm] (agentN) {Agent N\\CNNGRU};

\begin{scope}[on background layer]
\node[envbox, fill=fedblue!5, fit=(agent1)(agent2)(agent3)(agentN), label={[font=\small\bfseries, text=fedblue!80!black]below:Tier-1: Local PPO Agents (Non-IID Partitions)}] (agentgroup) {};
\end{scope}

% Agg to Agents connections
...
\draw[arrowstyle=fedblue!60, <->] (dynattn.south) -- ++(0,-0.5) -- (aggmid) |- (agentN.north);

% === Meta Agent (right side) ===
\node[metabox, right=2.5cm of server, minimum width=2.8cm] (meta) {Tier-2\\Meta-Agent\\(CNNGRUActor)};
\draw[arrowstyle=metapurple!70, <->] (server.east) -- (meta.west);
\draw[dashedarrow=metapurple!50] (meta.south) -- ++(0,-4.5) -| (agentN.east);
...
\caption{Overall architecture of FedRL-IDS: Tier-1 (PPO agents with CNNGRUActor backbone), Tier-2 (Meta-Agent coordinator), Tier-3 (Bernoulli client selector). The central server runs the tri-technique aggregation pipeline (FLTrust + Fed+ + Dynamic Attention).}
...
+ \varepsilon \cdot \mathrm{novelty\_bonus}
...
\textbf{Novelty bonus}: When a sample from an unseen class is correctly detected, the autoencoder reconstruction error exceeds threshold $\tau$:
\begin{equation}
r_{\mathrm{nov}} = \min\!\left(\frac{e(x)}{\tau} - 1,\ 1.0\right)
\end{equation}
...
\subsection{Autoencoder-based Novelty Detection}

An autoencoder is trained on benign/high-confidence traffic from the training set:
\begin{itemize}
    \item \textbf{Encoder}: $\mathbb{R}^d \to \mathbb{R}^{\max(64, d/2)} \to \mathbb{R}^{32}$ with ReLU
    \item \textbf{Decoder}: $\mathbb{R}^{32} \to \mathbb{R}^{\max(64, d/2)} \to \mathbb{R}^d$ (symmetric)
    \item \textbf{Loss}: MSE reconstruction error
\end{itemize}

The autoencoder is periodically retrained (every $N$ rounds) using high-confidence ($>0.9$) predictions from the current global model as pseudo-labels, ensuring the novelty boundary adapts to evolving attack patterns.
