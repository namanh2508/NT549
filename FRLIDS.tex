...
Recent domain-specific studies further motivate a broader comparison than the original baseline alone. PriFED-IDS investigates privacy-preserving federated reinforcement learning for digital health intrusion detection \cite{prifed_ids}. In industrial IoT, GRU-assisted federated learning combined with deep reinforcement learning-based device selection improves intrusion detection under heterogeneous traffic conditions \cite{gru_fl}. Sfedrl-IDS focuses on secure federated deep reinforcement learning for agricultural IoT and emphasizes secure collaborative training \cite{sfedrl_ids}. In highly dynamic wireless environments, reinforcement learning has also been used for drone-client selection in federated IDS for FANETs \cite{fanet_rl}. Collectively, these studies show that federated RL is promising across domains, but robustness, non-IID correction, and scalable response design are still mostly addressed separately rather than within a unified IDS framework.
...
\begin{enumerate}
    \item \textbf{Byzantine Resilience}: Most existing works emphasize privacy preservation or client efficiency, but do not explicitly defend against malicious model updates. Among the compared federated RL systems, Sfedrl-IDS is one of the few studies that more directly emphasizes secure collaborative learning \cite{sfedrl_ids}.
    ...
    \item \textbf{Combined Non-IID + Byzantine}: Existing studies typically focus on either non-IID robustness, privacy preservation, secure aggregation, or adaptive client selection. A unified integration of dynamic attention, FLTrust-style trust filtering, and Fed+-style server momentum remains insufficiently explored for IDS-oriented federated RL.
...
Our proposed FDRL-IDS addresses these gaps through a unified framework combining policy-based reinforcement learning, trust-aware federated aggregation, non-IID-aware server momentum, and scalable decision-making with online adaptation.
...
Cross-domain results reveal that FPR=1.0 indicates the model is heavily biased toward the attack class, likely due to severe distribution shift between IoT and IIoT datasets. This behavior is consistent with prior IDS literature showing that machine learning models often degrade sharply outside the original closed-world distribution \cite{sommer}. Therefore, the cross-domain experiment should be interpreted as a stress test that highlights the need for online adaptation and scalable decision-making, rather than as a fully solved generalization setting.
...
S. Vadigi, K. Sethi, D. Mohanty, S. P. Das, and P. Bera,
``Federated reinforcement learning based intrusion detection system using dynamic attention mechanism,''
\textit{Journal of Information Security and Applications}, vol. 78, Art. no. 103608, 2023.
...
% Domain-specific federated RL IDS references
\bibitem{prifed_ids}
S. Fu, H. Xu, A. Ali, and S. Sajid,
...
\textit{Electronics}, vol. 14, no. 23, Art. no. 4590, 2025.
...
IEEE article manuscript, 2024.

\bibitem{sfedrl_ids}
R. Benameur and A. Dahane,
...
Springer journal article, 2025.

\bibitem{fanet_rl}
S. Benfriha, N. Labraoui, H. A. B. Salameh, and R. Bensaid,
