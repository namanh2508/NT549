...
\begin{enumerate}
    \item \textbf{Dynamic Attention}: Compute attention weights based on agent accuracy
    \item \textbf{FLTrust Filtering}: Apply cosine similarity filtering to exclude Byzantine agents
    \item \textbf{Fed+ Momentum}: Apply server-side momentum to handle non-IID distributions
\end{enumerate}
...
The key advantage is that when a novel attack is encountered:
1. The system classifies it as UNKNOWN with default severity 2
2. Exploration finds effective responses through feedback
3. Learned responses are cached for future encounters
4. No manual retraining or hardcoding required
...
For novel attacks not matching known signatures, the UNKNOWN category (severity=2) is assigned, triggering an exploration mode:

\begin{itemize}
    \item The agent starts with optimistic Q-values for moderate blocking responses
    \item Exploration bonus encourages trying different response magnitudes
    \item After sufficient interactions (default: 10), the learned Q-values guide exploitation
\end{itemize}

The reward for continuous actions incorporates:
\begin{enumerate}
    \item \textbf{Base reward}: Appropriateness of action magnitude for the attack category
    \item \textbf{Learned component}: Q-value estimate from online learning
    \item \textbf{Cumulative impact penalty}: Discourages over-blocking legitimate traffic
\end{enumerate}
...
Key observations:
\begin{itemize}
    \item PPO achieves significantly higher AUC-ROC (+4.7\% over DQN) on NSL-KDD, indicating better discrimination ability
    \item Attention mechanism improves accuracy dramatically (+14.7\% for DQN)
    \item PPO with attention achieves near-perfect recall (99.47\%) but at the cost of higher FPR
    \item The combined aggregation method maintains high recall while providing Byzantine resilience
\end{itemize}
...
All improvements maintain federated learning's privacy guarantees:
\begin{itemize}
    \item Raw data never leaves local agents
    \item Only model weights (gradients) are shared with the server
    \item FLTrust's proxy dataset can be synthetically generated or use publicly available baseline data
\end{itemize}

\subsection{Limitations}

\begin{itemize}
    \item Cross-domain generalization remains challenging due to feature distribution shift
    \item The scalability of online learning depends on having sufficient exploration episodes
    \item Byzantine resilience assumes the server and proxy dataset are not compromised
\end{itemize}
